...
image.names <- sprintf("%07d.jpg", 1:200)

access_log$search.session <- sapply(query.params, function(l) l[2])
access_log$chosen <- factor(sapply(query.params, function(l) l[3]), image.names)
access_log$not.chosen <- factor(sapply(query.params, function(l) l[4]), image.names)
```
