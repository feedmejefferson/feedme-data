...
#meta = meta %>% filter(!(id %in% most.redundant))
...
write(jsonlite::toJSON(foodspace.json), "moderator/foodspace.json")

#all.tags = meta %>% 
#  mutate(tag.type="allTags") %>% distinct() %>% union(meta) %>% 
foods.json = meta %>% select(c(-original.tag)) %>% 
  left_join(edited) %>% 
  mutate(
